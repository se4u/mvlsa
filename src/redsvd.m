function [U,S,V] = redsvd(A,k,its,l)
%REDSVD  Low-rank approximation in SVD form.
%
%
%   [U,S,V] = REDSVD(A)  constructs a nearly optimal rank-6 approximation
...
%             the only input to REDSVD.
%
%   [U,S,V] = REDSVD(A,k)  constructs a nearly optimal rank-k approximation
%             USV' to A, using 2 full iterations of a block Lanczos method
%             of block size k+2, started with an n x (k+2) random matrix,
%             when A is m x n; the ref. below explains "nearly optimal."
%             k must be a positive integer <= the smallest dimension of A.
%
%   [U,S,V] = REDSVD(A,k,its)  constructs a nearly optimal rank-k approx. USV'
...
%
%   [U,S,V] = REDSVD(A,k,its,l)  constructs a nearly optimal rank-k approx.
...
%
%
%   Note: REDSVD invokes RAND. To obtain repeatable results,
%         invoke RAND('seed',j) with a fixed integer j before invoking REDSVD.
%
%   Note: REDSVD currently requires the user to center and normalize the rows
%         or columns of the input matrix A before invoking REDSVD (if such
%         is desired).
%
...
%
%
%   See also REDSVDCOV, PRINCOMP, SVDS.
%
...
%
if(nargin < 1)
  error('MATLAB:redsvd:TooFewIn',...
        'There must be at least 1 input.')
end

if(nargin > 4)
  error('MATLAB:redsvd:TooManyIn',...
...
%
if(nargout ~= 3)
  error('MATLAB:redsvd:WrongNumOut',...
...
%
if(~isfloat(A))
  error('MATLAB:redsvd:In1NotFloat',...
        'Input 1 must be a floating-point matrix.')
end

if(isempty(A))
  error('MATLAB:redsvd:In1Empty',...
...
if(size(k,1) ~= 1 || size(k,2) ~= 1)
  error('MATLAB:redsvd:In2Not1x1',...
        'Input 2 must be a scalar.')
end

if(size(its,1) ~= 1 || size(its,2) ~= 1)
  error('MATLAB:redsvd:In3Not1x1',...
        'Input 3 must be a scalar.')
end

if(size(l,1) ~= 1 || size(l,2) ~= 1)
  error('MATLAB:redsvd:In4Not1x1',...
        'Input 4 must be a scalar.')
end

if(k <= 0)
  error('MATLAB:redsvd:In2NonPos',...
        'Input 2 must be > 0.')
end

if((k > m) || (k > n))
  error('MATLAB:redsvd:In2TooBig',...
        'Input 2 must be <= the smallest dimension of Input 1.')
end

if(its < 0)
  error('MATLAB:redsvd:In3Neg',...
        'Input 3 must be >= 0.')
end

if(l < k)
  error('MATLAB:redsvd:In4ltIn2',...
